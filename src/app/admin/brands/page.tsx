...
export default function AdminBrandsPage() {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        const res = await catalogService.brands.getAll();
        setBrands(res.data);
        setIsLoading(false);
    };
    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (drawerMode === "CREATE") {
            res = await catalogService.brands.create(payload);
        } else if (selectedItem) {
            res = await catalogService.brands.update(selectedItem.id, payload);
        }
    ...
    const handleDelete = async () => {
        ...
        setIsSaving(true);
        const res = await catalogService.brands.delete(selectedItem.id);
        setIsSaving(false);
        if (res.success) {
            toast.success(res.message);
                ...
                <div className="overflow-x-auto min-h-[300px]">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center p-12 text-gray-400">
                            ...
                            <p className="font-plus-jakarta text-sm">Loading brands...</p>
                        </div>
                    ) : (
                        ...
                        <div className="border-t border-gray-100 bg-gray-50/50 p-6 dark:border-gray-800 dark:bg-[#1a1a1a]/50">
                            <div className="flex gap-3">
                                ...
                                <button disabled={isSaving} type="submit" className="flex-1 rounded-xl bg-blue-600 py-3 font-plus-jakarta text-sm font-bold text-white shadow-sm hover:bg-blue-700 active:scale-95 disabled:opacity-50 flex justify-center items-center">
                                    {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : (drawerMode === "CREATE" ? "Add Brand" : "Save Changes")}
                                </button>
                            </div>
                        </div>
                    </form>
                    ...
                    <div className="relative w-full max-w-md scale-100 rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#161616] border border-gray-100 dark:border-gray-800">
                        ...
                        <div className="mt-5 text-center">
                            ...
                            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                This action is high-risk. There might be products linked to this brand which will be affected.
                            </p>
                        </div>
