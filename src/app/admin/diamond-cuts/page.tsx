...
export default function AdminDiamondCutsPage() {
    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (drawerMode === "CREATE") {
            res = await specService.diamondSubTypes.create(payload);
        } else if (selectedItem) {
            res = await specService.diamondSubTypes.update(selectedItem.id, payload);
        }
        ...
        if (res?.success) {
            ...
            toast.error(res?.message || "Failed to save cut style");
        }
    };
    ...
    const handleDelete = async () => {
        ...
        setIsSaving(true);
        const res = await specService.diamondSubTypes.delete(selectedItem.id);
        setIsSaving(false);
        if (res.success) {
            toast.success(res.message);
            ...
            toast.error(res.message || "Failed to delete cut style");
        }
    };
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 z-10 rounded-xl border border-gray-100 bg-white p-2 shadow-xl dark:border-gray-800 dark:bg-[#1a1a1a]">
                            <div className="flex flex-col gap-1">
                                {(["ALL", "ACTIVE", "INACTIVE"] as const).map((status) => (
                                    ...
                                    >
                                        {status === "ALL" ? "All Shapes" : status === "ACTIVE" ? "Active Only" : "Inactive Only"}
                                    </button>
                                ))}
                            </div>
                        </div>
                ...
                <div className="overflow-x-auto min-h-[300px]">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center p-12 text-gray-400">
                            ...
                            <p className="font-plus-jakarta text-sm">Loading cut shapes...</p>
                        </div>
                    ) : (
