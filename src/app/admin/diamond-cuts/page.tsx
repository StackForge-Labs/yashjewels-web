...
const mockData: DiamondCutData[] = [
    { id: "IDX-001", name: "Princess Cut", description: "Classic square cut with pointed corners.", isActive: true, productCount: 42 },
    { id: "IDX-002", name: "Round Brilliant", description: "Standard 58-facet cut for maximum brilliance.", isActive: true, productCount: 15 },
];
...
export default function AdminDiamondCutsPage() {
    ...
    const [selectedItem, setSelectedItem] = useState<DiamondCutData | null>(null);
    ...
    const openEdit = (item: DiamondCutData) => {
        setDrawerMode("EDIT");
        setSelectedItem(item);
        setIsDrawerOpen(true);
    };

    const openDelete = (item: DiamondCutData) => {
        setSelectedItem(item);
            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p className="mt-1 font-plus-jakarta text-sm text-gray-500 dark:text-gray-400">
                        Manage diamond shapes and cutting precision standards.
                    </p>
                </div>
                <button 
                    onClick={openCreate}
                    ...
                    <Plus className="h-4 w-4" /> Add Cut Shape
                </button>
            </div>
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 z-10 rounded-xl border border-gray-100 bg-white p-2 shadow-xl dark:border-gray-800 dark:bg-[#1a1a1a]">
                            <div className="flex flex-col gap-1">
                                {(["ALL", "ACTIVE", "INACTIVE"] as const).map((status) => (
                                    ...
                                    >
                                        {status === "ALL" ? "All Designs" : status === "ACTIVE" ? "Active Only" : "Inactive Only"}
                                    </button>
                                ))}
                            </div>
                        </div>
                    ...
                    <table className="w-full whitespace-nowrap text-left text-sm">
                        <thead className="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800/50 dark:bg-[#111]/50">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-plus-jakarta text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 max-w-[200px]">Technical Description</th>
                    ...
                    <div className="relative w-full max-w-md bg-white shadow-2xl dark:bg-[#111] flex flex-col h-full border-l border-gray-100 dark:border-gray-800">
                        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5 dark:border-gray-800">
                            <div>
                                ...
                                <p className="font-plus-jakarta text-xs text-gray-500 mt-1">Manage technical diamond shapes and cutting precision.</p>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-6 font-plus-jakarta">
                            <div className="flex flex-col gap-5">
                                ...
                                <div>
                                    <label className="mb-1.5 block text-xs font-bold text-gray-700 dark:text-gray-300">Technical Details</label>
                                    <textarea 
                                        rows={3}
                                        defaultValue={selectedItem?.description || ""}
                                        placeholder="Faceting and geometric details..."
                                        className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:border-blue-500 focus:bg-white focus:outline-none dark:border-gray-800 dark:bg-[#1a1a1a] dark:text-white dark:focus:border-blue-500"
                                    />
                                </div>
                                <div className="flex items-center justify-between rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">Customers will see this shape in the search filters.</p>
                                    </div>
                        ...
                        <div className="border-t border-gray-100 bg-gray-50/50 p-6 dark:border-gray-800 dark:bg-[#1a1a1a]/50">
                            <div className="flex gap-3">
                                ...
                                <button className="flex-1 rounded-xl bg-blue-600 py-3 font-plus-jakarta text-sm font-bold text-white shadow-sm hover:bg-blue-700 active:scale-95">
                                    {drawerMode === "CREATE" ? "Add Cut Shape" : "Save Changes"}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative w-full max-w-md scale-100 rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#161616] border border-gray-100 dark:border-gray-800">
                        ...
                        <div className="mt-5 text-center">
                            ...
                            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                This action is high-risk. There are <strong className="text-rose-500">{selectedItem.productCount} items</strong> tied to this specific cut shape.
                            </p>
                        </div>
