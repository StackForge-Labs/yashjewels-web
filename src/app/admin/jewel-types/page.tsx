...
export default function AdminJewelTypesPage() {
    const [jewelTypes, setJewelTypes] = useState<CatalogItem[]>([]);
    ...
    const fetchData = async () => {
        setIsLoading(true);
        const res = await catalogService.jewelTypes.getAll();
    ...
    const filtered = jewelTypes.filter(d => {
    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (drawerMode === "CREATE") {
            res = await catalogService.jewelTypes.create(payload);
        } else if (selectedItem) {
            res = await catalogService.jewelTypes.update(selectedItem.id, payload);
        }
        ...
        if (res?.success) {
            ...
            toast.error(res?.message || "Failed to save jewel type");
        }
    };
    ...
    const handleDelete = async () => {
        ...
        setIsSaving(true);
        const res = await catalogService.jewelTypes.delete(selectedItem.id);
        setIsSaving(false);
        if (res.success) {
            toast.success(res.message);
            ...
            toast.error(res.message || "Failed to delete jewel type");
        }
    };
            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p className="mt-1 font-plus-jakarta text-sm text-gray-500 dark:text-gray-400">
                        Manage customer segments and gender-specific classifications.
                    </p>
                </div>
                <button 
                    onClick={openCreate}
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 z-10 rounded-xl border border-gray-100 bg-white p-2 shadow-xl dark:border-gray-800 dark:bg-[#1a1a1a]">
                            <div className="flex flex-col gap-1">
                                {(["ALL", "ACTIVE", "INACTIVE"] as const).map((status) => (
                                    ...
                                    >
                                        {status === "ALL" ? "All Segments" : status === "ACTIVE" ? "Active Only" : "Inactive Only"}
                                    </button>
                                ))}
                            </div>
                        </div>
                ...
                <div className="overflow-x-auto min-h-[300px]">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center p-12 text-gray-400">
                            ...
                            <p className="font-plus-jakarta text-sm">Loading segments...</p>
                        </div>
                    ) : (
                        <table className="w-full whitespace-nowrap text-left text-sm">
                            <thead className="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800/50 dark:bg-[#111]/50">
                                <tr>
                                    <th className="px-6 py-4 font-plus-jakarta text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500">Segment Name</th>
                                    <th className="px-6 py-4 font-plus-jakarta text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 max-w-[200px]">Target Audience</th>
                        ...
                        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5 dark:border-gray-800">
                            <div>
                                <h2 className="font-serif text-xl font-bold dark:text-white">
                                    {drawerMode === "CREATE" ? "New Segment" : "Update Segment"}
                                </h2>
                                <p className="font-plus-jakarta text-xs text-gray-500 mt-1">Manage target consumer demographics and gender segments.</p>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-5">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        placeholder="Enter name (e.g. Women's Collection)..."
                                ...
                                <div>
                                    <label className="mb-1.5 block text-xs font-bold text-gray-700 dark:text-gray-300">Segment Description</label>
                                    <textarea 
                                        name="description"
                                        rows={3}
                                        defaultValue={selectedItem?.description || ""}
                                        placeholder="Target audience and demographic details..."
                                        className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm focus:border-blue-500 focus:bg-white focus:outline-none dark:border-gray-800 dark:bg-[#1a1a1a] dark:text-white dark:focus:border-blue-500"
                                    />
                                </div>
                                {drawerMode === "EDIT" && (
                                    <div className="flex items-center justify-between rounded-xl border border-gray-200 p-4 dark:border-gray-800">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">Customers will see this segment in catalog navigation.</p>
                                        </div>
                        ...
                        <div className="border-t border-gray-100 bg-gray-50/50 p-6 dark:border-gray-800 dark:bg-[#1a1a1a]/50">
                            <div className="flex gap-3">
                                ...
                                <button disabled={isSaving} type="submit" className="flex-1 rounded-xl bg-blue-600 py-3 font-plus-jakarta text-sm font-bold text-white shadow-sm hover:bg-blue-700 active:scale-95 disabled:opacity-50 flex justify-center items-center">
                                    {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : (drawerMode === "CREATE" ? "Add Segment" : "Save Changes")}
                                </button>
                            </div>
                        </div>
                    </form>
                    ...
                    <div className="relative w-full max-w-md scale-100 rounded-3xl bg-white p-6 shadow-2xl dark:bg-[#161616] border border-gray-100 dark:border-gray-800">
                        ...
                        <div className="mt-5 text-center">
                            ...
                            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                This action is high-risk. There might be products assigned to this target segment.
                            </p>
                        </div>
