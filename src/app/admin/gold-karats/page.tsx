...
export default function AdminGoldKaratsPage() {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        const res = await specService.goldKarats.getAll();
        setGoldKarats(res.data);
        setIsLoading(false);
    };
    ...
    const filtered = goldKarats.filter(d => {
        const label = d.caratLabel || "";
    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const payload = {
            caratLabel: formData.get("caratLabel") as string,
            karatValue: parseFloat(formData.get("karatValue") as string),
            isActive: drawerMode === "EDIT" ? formData.get("isActive") === "on" : undefined,
        };

        let res;
        if (drawerMode === "CREATE") {
            res = await specService.goldKarats.create(payload);
        } else if (selectedItem) {
            res = await specService.goldKarats.update(selectedItem.id, payload);
        }
        ...
        if (res?.success) {
            ...
            toast.error(res?.message || "Failed to save gold karat");
        }
    };
    ...
    const handleDelete = async () => {
        ...
        setIsSaving(true);
        const res = await specService.goldKarats.delete(selectedItem.id);
        setIsSaving(false);
        if (res.success) {
            toast.success(res.message);
            ...
            toast.error(res.message || "Failed to delete gold karat");
        }
    };
                ...
                <div className="overflow-x-auto min-h-[300px]">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center p-12 text-gray-400">
                            ...
                            <p className="font-plus-jakarta text-sm">Loading gold karats...</p>
                        </div>
                    ) : (
                                ...
                                {filtered.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="px-6 py-12 text-center text-gray-500 font-plus-jakarta text-sm">
                                            No karats found matching your filters.
                                        </td>
                                    </tr>
                                ) : filtered.map((item) => (
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="w-full max-w-[200px]">
                                                <div className="mb-1 flex justify-between font-plus-jakarta text-xs font-semibold">
                                                    <span className="text-gray-900 dark:text-gray-200">{item.karatValue}%</span>
                                                    <span className="text-gray-400">Purity Level</span>
                                                </div>
                                                <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
                                                    <div 
                                                        className="h-full rounded-full bg-gradient-to-r from-amber-400 to-amber-600"
                                                        style={{ width: `${item.karatValue}%` }}
                                                    />
                                                </div>
                                            </div>
                                        </td>
                    ...
                    <form onSubmit={handleSave} className="relative w-full max-w-md bg-white shadow-2xl dark:bg-[#111] flex flex-col h-full border-l border-gray-100 dark:border-gray-800">
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-6 font-plus-jakarta">
                            <div className="flex flex-col gap-5">
                                ...
                                <div>
                                    <label className="mb-1.5 block text-xs font-bold text-gray-700 dark:text-gray-300">
                                        Gold Content (%) <span className="text-rose-500">*</span>
                                    </label>
                                    <input 
                                        type="number" 
                                        name="karatValue"
                                        step="0.01"
                                        required
                                        defaultValue={selectedItem?.karatValue || ""}
